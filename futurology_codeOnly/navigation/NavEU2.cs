...
public class NavEU2 : MonoBehaviour {
	...
	void Start () {
		...
		uav2AnchorPos = this.transform.position;
	...
	void Update () {
		...
		if (retreatLocked == true){
			...
			agent.SetDestination (uav2AnchorPos);
			//Debug.Log (uav2AnchorPos);
			//float distanceBack = Vector3.Distance (this.transform.position, uav1AnchorPos);
			retreat = false;
			retreatLocked = false;
		}
		if (HPUAVE2.damageDealing <= 1.0f) {
			agent.Stop ();
		}
