...
public class NavRhinoE : MonoBehaviour {
	...
	void Start () {
		...
		rhino1AnchorPos = this.transform.position;
	...
	void Update () {
		...
		if (retreatLocked == true){
			...
			agent.SetDestination(rhino1AnchorPos);
			running = 3.0f;
			attacking = 1.0f;
		...
		if (HPRhinoE.damageDealing <= 1.0f){
			hitting = 1.0f;
			running = 1.0f;
			attacking = 1.0f;
			dying = 3.0f;
